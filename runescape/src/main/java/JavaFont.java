...
public final class JavaFont extends Font {
    ...
    public JavaFont(@OriginalArg(0) JavaToolkit toolkit, @OriginalArg(1) FontMetrics metrics, @OriginalArg(2) IndexedImage[] glyphs, @OriginalArg(3) int[] glyphWidth, @OriginalArg(4) int[] glyphHeight) {
    ...
    protected void renderSymbol(@OriginalArg(0) char c, @OriginalArg(1) int x, @OriginalArg(2) int y, @OriginalArg(3) int colour, @OriginalArg(4) boolean shadow, @OriginalArg(5) ClippingMask mask, @OriginalArg(6) int offsetX, @OriginalArg(7) int offsetY) {
        ...
        if (shadow) {
            this.blitSolidMasked(this.glyphRaster[c], this.toolkit.surfaceRaster, colour, srcIndex, dstIndex, width, height, dstStep, srcStep, x, y, this.glyphWidth[c], mask, offsetX, offsetY);
        } else {
            this.blitPaletteMasked(this.glyphRaster[c], this.toolkit.surfaceRaster, this.palette, colour, srcIndex, dstIndex, width, height, dstStep, srcStep, x, y, this.glyphWidth[c], mask, offsetX, offsetY);
        }
    }

    @OriginalMember(owner = "client!ib", name = "a", descriptor = "([B[IIIIIIII)V")
    public void blitSolid(@OriginalArg(0) byte[] src, @OriginalArg(1) int[] dst, @OriginalArg(2) int colour, @OriginalArg(3) int srcIndex, @OriginalArg(4) int dstIndex, @OriginalArg(5) int width, @OriginalArg(6) int height, @OriginalArg(7) int dstStep, @OriginalArg(8) int srcStep) {
    ...
    public void blitPalette(@OriginalArg(0) byte[] src, @OriginalArg(1) int[] dst, @OriginalArg(2) int[] palette, @OriginalArg(3) int srcIndex, @OriginalArg(4) int dstIndex, @OriginalArg(5) int width, @OriginalArg(6) int height, @OriginalArg(7) int dstStep, @OriginalArg(8) int srcStep) {
        @Pc(4) int blocks = -(width >> 2);
        @Pc(9) int remainder = -(width & 0x3);
        for (@Pc(14) int row = -height; row < 0; row++) {
            for (@Pc(17) int block = blocks; block < 0; block++) {
                @Pc(24) byte pixel0;
                if ((pixel0 = src[srcIndex++]) == 0) {
                    dstIndex++;
                } else {
                    dst[dstIndex++] = palette[pixel0 & 0xFF];
                }
                @Pc(42) byte pixel1;
                if ((pixel1 = src[srcIndex++]) == 0) {
                    dstIndex++;
                } else {
                    dst[dstIndex++] = palette[pixel1 & 0xFF];
                }
                @Pc(60) byte pixel2;
                if ((pixel2 = src[srcIndex++]) == 0) {
                    dstIndex++;
                } else {
                    dst[dstIndex++] = palette[pixel2 & 0xFF];
                }
                @Pc(78) byte pixel3;
                if ((pixel3 = src[srcIndex++]) == 0) {
                    dstIndex++;
                } else {
                    dst[dstIndex++] = palette[pixel3 & 0xFF];
                }
            }
            for (@Pc(95) int column = remainder; column < 0; column++) {
                @Pc(102) byte pixel;
                if ((pixel = src[srcIndex++]) == 0) {
                    dstIndex++;
                } else {
                    dst[dstIndex++] = palette[pixel & 0xFF];
                }
            }
            dstIndex += dstStep;
            srcIndex += srcStep;
    ...
    public void blitPaletteMasked(@OriginalArg(0) byte[] src, @OriginalArg(1) int[] dst, @OriginalArg(2) int[] palette, @OriginalArg(3) int colour, @OriginalArg(4) int srcIndex, @OriginalArg(5) int dstIndex, @OriginalArg(6) int width, @OriginalArg(7) int height, @OriginalArg(8) int dstStep, @OriginalArg(9) int srcStep, @OriginalArg(10) int x, @OriginalArg(11) int y, @OriginalArg(12) int glyphWidth, @OriginalArg(13) ClippingMask mask, @OriginalArg(14) int maskX, @OriginalArg(15) int maskY) {
        ...
        for (@Pc(60) int row = startY; row < endY; row++) {
            @Pc(69) int lineX = lineOffsets[row - maskY] + maskX;
            @Pc(75) int lineWidth = lineWidths[row - maskY];
            @Pc(77) int remaining = width;
            @Pc(84) int skip;
            ...
            for (@Pc(152) int column = -lineWidth; column < 0; column++) {
                @Pc(159) byte pixel;
                if ((pixel = src[srcIndex++]) == 0) {
                    dstIndex++;
                } else {
    ...
    protected void fa(@OriginalArg(0) char c, @OriginalArg(1) int x, @OriginalArg(2) int y, @OriginalArg(3) int colour, @OriginalArg(4) boolean shadow) {
        ...
        if (shadow) {
            this.blitSolid(this.glyphRaster[c], this.toolkit.surfaceRaster, colour, srcIndex, dstIndex, width, height, dstStep, srcStep);
        } else {
            this.blitPalette(this.glyphRaster[c], this.toolkit.surfaceRaster, this.palette, srcIndex, dstIndex, width, height, dstStep, srcStep);
        }
    }
}
