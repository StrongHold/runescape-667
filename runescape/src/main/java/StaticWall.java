...
public final class StaticWall extends Wall implements Location {
    ...
    public static final int[] CORNER_MASKS = new int[]{16, 32, 64, 128};

    @OriginalMember(owner = "client!gn", name = "i", descriptor = "[I")
    public static final int[] SIDE_MASKS = new int[]{1, 2, 4, 8};

    @OriginalMember(owner = "client!pha", name = "a", descriptor = "(ZII)I")
    public static int sideMask(@OriginalArg(1) int rotation, @OriginalArg(2) int shape) {
        if (shape == LocShapes.WALL_DIAGONALCORNER || shape == LocShapes.WALL_SQUARECORNER) {
            return CORNER_MASKS[rotation & 0x3];
        } else {
            return SIDE_MASKS[rotation & 0x3];
        }
    }
    ...
    public StaticWall(@OriginalArg(0) Toolkit toolkit, @OriginalArg(1) LocType type, @OriginalArg(2) int level, @OriginalArg(3) int virtualLevel, @OriginalArg(4) int x, @OriginalArg(5) int y, @OriginalArg(6) int z, @OriginalArg(7) boolean underwater, @OriginalArg(8) int shape, @OriginalArg(9) int rotation, @OriginalArg(10) boolean copyNormals) {
        super(x, y, z, level, virtualLevel, sideMask(rotation, shape));
        super.x = x;
        super.z = z;
    ...
    public void addShadow(@OriginalArg(0) Toolkit toolkit) {
        ...
        if (this.shadow == null && this.hardShadow) {
            @Pc(32) ModelAndShadow modelAndShadow = this.modelAndShadow(toolkit, 262144, true);
    ...
    public Model getModel(@OriginalArg(0) Toolkit toolkit, @OriginalArg(2) int functionMask) {
        if (this.model != null && toolkit.compareFunctionMasks(this.model.ua(), functionMask) == 0) {
            return this.model;
        } else {
            @Pc(37) ModelAndShadow modelAndShadow = this.modelAndShadow(toolkit, functionMask, false);
    ...
    public void removeShadow(@OriginalArg(0) Toolkit toolkit) {
        @Pc(24) Shadow shadow;
        if (this.shadow == null && this.hardShadow) {
            @Pc(35) ModelAndShadow modelAndShadow = this.modelAndShadow(toolkit, 262144, true);
            shadow = modelAndShadow == null ? null : modelAndShadow.shadow;
        } else {
            shadow = this.shadow;
            this.shadow = null;
    ...
    public ModelAndShadow modelAndShadow(@OriginalArg(0) Toolkit toolkit, @OriginalArg(2) int functionMask, @OriginalArg(3) boolean addShadow) {
        ...
        @Pc(38) Ground ceiling = LocGround.ceiling(this.underwater, super.virtualLevel);
