...
public final class Static446 {
     ...
     * A tile holds the current value of {@link Static675#occlusionFrame} once it has been found hidden
    ...
    public static void fillAndOutlineRectUnclipped(@OriginalArg(0) int y1, @OriginalArg(1) int x1, @OriginalArg(2) int lineColour, @OriginalArg(4) int fillColour, @OriginalArg(5) int x0, @OriginalArg(6) int lineWidth, @OriginalArg(7) int y0) {
        ...
        for (@Pc(21) int y = y0; y < topEnd; y++) {
            Static696.fillHorizontalSpan(x1, lineColour, x0, Static723.anIntArrayArray266[y]);
        }
        ...
        for (@Pc(72) int y = topEnd; y <= bottomStart; y++) {
            ...
            Static696.fillHorizontalSpan(leftEnd, lineColour, x0, row);
            Static696.fillHorizontalSpan(rightStart, fillColour, leftEnd, row);
            Static696.fillHorizontalSpan(x1, lineColour, rightStart, row);
        }
    }
}
